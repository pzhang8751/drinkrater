...
function ReviewForm({ name, action, isOpen, onSubmit }: PopUp) {
  ...
  function onCommentChangeHandler(
  ...
  return (
    <div className={isOpen ? "" : "hidden"}>
      <div
        className="h-screen w-screen bg-black opacity-50 fixed top-0 left-0 z-30"
        onClick={action}
      ></div>
      <section className="w-[74%] px-3 pt-2 pb-3 bg-white fixed top-[7%] left-[13%] z-40 space-y-2">
        <div className="flex">
          ...
          <button
            className="font-semibold hover:text-red-500 justify-self-end"
            onClick={closeWindow}
          >
            X
          </button>
        ...
        <form id="reviewform" action={submitReview} className="px-5 space-y-2">
          ...
          <textarea
            rows={10}
            id="comment"
            className="px-1 w-full border-2 resize-none"
        ...
        <div className="px-5 flex">
          ...
          <button
            form="reviewform"
            className="px-5 py-2 rounded-full bg-orange-200 hover:bg-orange-400 hover:scale-90 transition-all duration-300 ease-in-out"
            type="submit"
          >
            Submit
          </button>
...
function StarRating({ stars, action }: Rating) {
  ...
  return (
    <div
      ...
      id="starContainer"
      className="w-min flex hover:cursor-pointer"
    >
      {starArray}
    </div>
  );
